            ...
            >
                ...
                <div class="flex-1 flex overflow-hidden">
                    ...
                    <div class="flex-1 p-4 overflow-y-auto">
                        <div class="mb-4">
                            <h3 class="text-[10px] font-bold text-blue-400/40 uppercase tracking-[0.2em] mb-3">Priority
                                Applications</h3>
                                ...
                                <For each={filteredApps()}>
                                    {(app) => (
                                        <button
                                            ...
                                            onClick={() => {
                                                props.onAppLaunch(app);
                                                props.onClose();
                                            }}
                                        >
                                            <div
                                                class="w-12 h-12 mb-2 flex items-center justify-center text-blue-400 group-hover:scale-110 group-hover:drop-shadow-[0_0_8px_rgba(96,165,250,0.5)] transition-all">
                                                {getIconComponent(app.icon)}
                                            </div>
                                            <span
                                                class="text-[10px] text-blue-400 font-medium uppercase tracking-tighter text-center">{app.name}</span>
                                        </button>
                                    )}
                                </For>
                            </div>
                    ...
                    {/* Sidebar */}
                    <div
                        class="w-16 bg-blue-400/5 border-l border-blue-400/10 flex flex-col items-center py-4 space-y-6">
                        <Settings size={18}
                                  class="text-blue-400/40 hover:text-blue-400 cursor-pointer transition-colors"/>
                        <Folder size={18}
                                class="text-blue-400/40 hover:text-blue-400 cursor-pointer transition-colors"/>
                        <Monitor size={18}
                                 class="text-blue-400/40 hover:text-blue-400 cursor-pointer transition-colors"/>
                ...
                <div
                    class="pl-4 pr-3 py-2 bg-blue-400/10 border-y border-blue-400/20 flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <div
                            class="size-8 rounded-full border-2 border-blue-400 p-0.5 shadow-[0_0_15px_rgba(96,165,250,0.5)]">
                            <div
                                class="w-full h-full rounded-full bg-blue-900 flex items-center justify-center text-blue-400">
                                <User size={24}/>
                            </div>
                        </div>
                        <div>
                            <h2 class="text-[14px] font-bold text-blue-400 tracking-wider uppercase leading-4">{props.username}</h2>
                            <p class="text-[10px] text-blue-400/60 font-mono mt-1 tracking-tighter uppercase">Administrator
                                Access Granted</p>
                        </div>
                    </div>
                    <Button variant="ghost" size="icon" class="text-red-400 hover:bg-red-400/10 rounded-full"
                ...
                <div class="p-3 bg-blue-400/5 border-t border-blue-400/10 flex items-center justify-between px-6">
                    ...
                    <span
                        class="text-[9px] font-mono text-blue-400/40 uppercase tracking-widest">BatOS v1.0.4-LQUND</span>
                </div>
            </div>
        </Show>
    )
